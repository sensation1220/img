...
try:
    ...
    model = genai.GenerativeModel('gemini-1.5-flash') # 使用するモデルを指定
...
def process_file_with_llm_agent(file_path):
    """LLMエージェントにファイル処理を依頼し、結果を取得し、画像を挿入する"""
    if model is None:
        print("エラー: Geminiモデルが設定されていません。処理をスキップします。")
        return None

    print(f"--- LLMエージェント処理開始: {os.path.basename(file_path)} ---")
...
4.  **ブロガー風リライト**: 全てのh3見出しを読者が読みたくなるようにリライトし、対応する本文も以下の口調でリライトしてください。
    -   冷静かつ理詰め。敬体だが淡々としすぎず、マニア視点で深掘る文体。専門的になりすぎないように注意。
    -   絵文字は使用しないでください。
...
"""
    ...
    if not romanized_folder_name:
        ...
        return processed_content # 画像なしでコンテンツを返す
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip().startswith("###"): # ###で始まる行を検出
            ...
            if h3_count == 1:
                ...
                # 2番目以降のh3にはeye.webp以外のランダムな画像を挿入（重複なし）
            ...
            if image_to_insert:
                ...
                output_lines.append("") # 画像文字列の後に空行を追加して整形

    print(f"--- LLMエージェント処理完了（画像挿入済み）: {os.path.basename(file_path)} ---\n")
    return "\n".join(output_lines)
...
def main():
    ...
    if args.resume:
        ...
        print("新規モードで実行します。")
        # 0-listからファイルをランダムに選択し、新しいディレクトリにコピー
    ...
    # 初回実行時またはprogress.txtが空の場合、開始時刻と総ファイル数を記録
    ...
    if not interrupted_file:
        start_processing = True # 中断ファイルがない場合は最初から開始
    ...
    for file_path in all_md_files:
        ...
        if not start_processing:
            if interrupted_file and filename == interrupted_file:
                ...
                # 中断ファイルがまだ見つかっていない、かつ処理済みでもないファイル
                continue
        ...
        # LLMエージェントによる処理
        processed_content = process_file_with_llm_agent(file_path)
    ...
    # 全てのファイルが処理されたか確認
    # 注意：この判定は不完全な可能性がある。load_progressを再利用してより正確に判定する。
